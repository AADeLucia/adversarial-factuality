...
@Scorer.register("llm-support-internal")
class LLMSupportInternalScorer(Scorer):

    __NAME__ = "llm-support-internal"
    ...
    def __init__(
        ...
        self._agent = FActScoreKnowledgeSupportStep().chain_llm(self._llm)

    @overrides
    def _score(self, instance: ScorerInstance) -> Dict[Text, Union[Text, float]]:
        """Score an instance based on internal knowledge support"""
        input_instance = {
            "input": instance.text
        }

        response = self._agent.invoke(input_instance, config=self._runnable_config)
        return {"raw": response.messages, "parsed": response.evidential_support, "support_input": None}
    ...
    def _batch_score(
        ...
        for response, input_instance in zip(responses, instances):
            results.append({
                ...
                "claim_sentence": input_instance.sentence,
                "support_input": None,
            })
